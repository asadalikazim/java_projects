...
public class DirectedGraphImpl implements Graph, Reversible {
  ...
  public String toString() {
    ...
    for (int i = 0; i < nodes_in_graph.size(); i++) {
      ...
      List<Node> connected_nodes = new ArrayList<Node>();
      List<Node> connection_from = new ArrayList<Node>();
      ...
      for (int j = 0; j < edges_in_graph.size(); j++) {
        ...
        if (edges_in_graph.get(j).getSecondNode().getId() == working_node.getId()) {
          connection_from.add(edges_in_graph.get(j).getFirstNode());
        }
      }
      ...
      for (int k = 0; k < connected_nodes.size(); k++) {
        ...
        String double_version = String.valueOf(working_node.getId()) + "<->"
            + String.valueOf(connected_nodes.get(k).getId()) + ";";

        if (!working_list.contains(other_pversion)) {
          working_list.add(current_string);
        } else {
          working_list.add(double_version);
          for (int m = 0; m < working_list.size(); m++) {
            if (working_list.get(m).equals(other_pversion)) {
              working_list.remove(m);
              break;
            }
          }
        }
      ...
      if (connection_from.size() == 0 && connected_nodes.size() == 0
  ...
  public void reverse() {
    ...
    while (edges_in_graph.size() > 0) {
      Edge current_edge = edges_in_graph.get(0);
      Edge new_edge = new EdgeImpl(current_edge.getSecondNode(), current_edge.getFirstNode());
      new_edges.add(new_edge);
      edges_in_graph.remove(0);
    }

    for (int i = 0; i < new_edges.size(); i++) {
      edges_in_graph.add(new_edges.get(i));
    }
  }
